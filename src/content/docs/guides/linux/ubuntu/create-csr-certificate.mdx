...
<MatchPairs
  instruction="Match the CSR component with its description"
  pairs={[
    { left: "CN", right: "Common Name (hostname)" },
    { left: "SAN", right: "Subject Alternative Names" },
    { left: "O", right: "Organization" },
    { left: "OU", right: "Organizational Unit" },
  ]}
/>

<Terminal
  commands={[
    { input: "openssl req -new -key server.key -out server.csr", output: "" },
  ]}
/>

<FillBlank
  instruction="Complete the SAN entry:"
  sentence="To add SAN entries, include subjectAltName = DNS:example.com, [BLANK:DNS]:www.example.com in the config"
/>
